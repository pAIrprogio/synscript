...
export class Str extends Pipeable<Str, string> {
  ...
  /**
   * Chain operations on the Str instance
   * @param fn - Function that takes a Str instance and returns a value
   * @returns The result of the function, or a new Pipe instance if the result is not a primitive
   * @example
   * ```typescript
   ...
   * ```
   */
  public override _<R>(fn: (value: Str) => R): R {
    return super._(fn);
  }

  /**
   * Chain operations on the underlying string value
   * @param fn - Function that takes a string value and returns a result
   * @returns The result of the function, or a new Pipe instance if the result is not a primitive
   * @example
   * ```typescript
   * // Transform the string value directly
   * str('hello')
   *   ._$((s) => s.toUpperCase())
   *   .$  // 'HELLO'
   *
   ...
   * ```
   */
  public override _$<R>(fn: (value: string) => R): R {
    return super._$(fn);
  }

  /**
   * Get the final string value from the chain
